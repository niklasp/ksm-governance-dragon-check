...
const GOV_DRAGON_RES_IPFS = 'bafkreihhdi4rhjmxhu5whr3dntx2mvl46jkj67jxpsp4cmo4kcbwybypau'
const GOV_DRAGON_EGG_RES_IPFS = 'bafkreihiw3lo62kjq2r2atot5lldkfxuavd4q6fvsmsdttvkkwlvux6nai'
...
const fetchGovNftsForAddress = async ( ksmAddress ) => {
  const endpoint = `https://rmrklistener.com/get_nfts_owned_by/${ ksmAddress }`;
  ...
  if ( typeof shelf !== 'undefined' ) {
    ...
    //check if the dragon, not the egg is equipped
    if ( typeof dragonNft !== 'undefined' ) {
      const dragonItem = await fetchNftById( dragonNft.id );
      console.log( 'DRAGON', dragonItem );
      const priorities = dragonItem.priority;

      console.log( 'prio', priorities );
      const dragonResourceIdx = priorities.findIndex( rid => {
        return dragonItem.resources.find( res => res.id === rid )?.src.includes(  GOV_DRAGON_RES_IPFS )
      });
      const eggResourceIdx = priorities.findIndex( rid => {
        return dragonItem.resources.find( res => res.id === rid )?.src.includes(  GOV_DRAGON_EGG_RES_IPFS )
      });
      
      if ( dragonResourceIdx < eggResourceIdx ) {
        result.dragonCorrect = true;
      }
    }
    ...
    console.log( 'relevant nfts', relevantNfts );
...
const fetchNftById = async ( id ) => {
  const endpoint = `https://rmrklistener.com/get_nft_by_id/${ id }`
...
const fetchGovNftsForWallet = async ( ksmAddress ) => {
  ...
  if ( typeof shelf !== 'undefined' ) {
    ...
    const nft_endpoint = `https://rmrklistener.com/get_nft_by_id/${ shelfId }`;
